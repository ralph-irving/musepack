...
#if 0
static const float  Puls [11] = {                                                       // aus MatLab-Simulation (Fouriertransformierte des Cos-Rolloff)
...
/*
// Generation des Cos-Rolloff der Cepstralanalyse, Cos-Rolloff von 5512,5 Hz bis 11025 Hz
// for ( k = 0; k <= 1024; k++ ) {
...
// setzt die Harmonischen und ihre Oberwellen
static void
SetVoiceLines ( unsigned char* VoiceLine, const float base, int val )
{
    int    n;
    int    max = (int) (MAX_CVD_LINE * base / 1024.f);  // Oberwellen bis zum Index MAX_CVD_LINE (Spektrallinien außerhalb machen keinen Sinn)
    int    line;
    float  frq = 1024.f / base;                         // frq = 1024./i ist Index der Grundharmonischen

    // Durchlaufe alle Oberwellen
    for ( n = 1; n <= max; n++ ) {
        line = (int) (n * frq);
...
// Analyse des Cepstrums, Suche nach Grundharmonischer
static void
CEP_Analyse2048 ( float* res1, float* res2, float *cep )
{
    ...
    // Kreuzkorrelation mit Pulsform
    // Berechnung von idx = MIN_ANALYZED_IDX-2  bis  MAX_ANALYZED_IDX+2,
    // da diese bei Suche nach Maximum gelesen werden
    // 50 -> 882 Hz, 700 -> 63 Hz Basisfrequenz
    ...
    *res1 = *res2 = 0. ;
    ...
    for ( n = MIN_ANALYZED_IDX - 2; n <= MAX_ANALYZED_IDX + 2; n++ ) {
        ...
        kkf  = x[-4] * Puls [0] +
               ...
               x[ 3] * Puls [7] +
               x[ 4] * Puls [8];
        cc [n] = norm > 0. ?  kkf * kkf / norm  :  0.f;         // um sqrt() einzusparen, wird das Quadrat des ncc berechnet
    }

    // Suche nach (relativem) Maximum
    ref  = 0.f;
    ...
    // Berechnung der mittleren Lage des Maximums (Interpolation)
    x        = cep + line;
...
static __inline float   /* This is an rough estimation with an accuracy of |x|<0.0037 */
logfast ( float x )
{
    double  y = x * x;
    y *= y;
...
// ClearVoiceDetection für Spektrum *spec
// input : Spektrum *spec
// output: Array *vocal enthält Information, ob FFT-Linie harmonische Komponente ist
int
CVD2048 ( const float* spec, unsigned char* vocal )
{
    static float  cep [4096];     // cep[4096] -- array, welches auch für 2048er FFT verwendet wird
    ...
    ENTER(20);
    // Berechne logarithmiertes, gefenstertes Spektrum cep[]
    // cep[512...1024] = 0 -- cep[1025...2047] ist egal, da diese erst durch fft gefüllt werden
    for ( n =   0; n < 256; n++ )
        cep[n] = logfast (*spec++);
    for ( n = 256; n < 512; n++ )
    ...
    // Berechne Cepstrum aus cep[] (Funktion Cepstrum() liefert das Cepstrum in-place)
    Cepstrum2048 ( cep, MAX_ANALYZED_IDX );

    // Suche Harmonische
    CEP_Analyse2048 ( &res1, &res2, cep );
    if ( res1 > 0.f  ||  res2 > 0.f ) {
        memset ( vocal, 0, (MAX_CVD_LINE + 4) * sizeof (*vocal) );
